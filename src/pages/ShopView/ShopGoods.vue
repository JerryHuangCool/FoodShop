...
export default {
    setup() {
        ...
        function _initScroll() {
            //初始化滚动条
            leftScroll = new BScroll(menuWrapper.value, {
                scrollY: true,
                click: true, // 默认不开启点击监听，需手动开启
            });
            rightScroll = new BScroll(foodsWrapper.value, {
                scrollY: true,
                click: true,
                probeType: 2,
            });
            rightScroll.on("scroll", (position) => {
        ...
        function clickMenuItem(index) {
            ...
            rightScroll.scrollTo(0, -tops.value[index], 300);
        }
        return {
            ...storeState,
            currentIndex,
